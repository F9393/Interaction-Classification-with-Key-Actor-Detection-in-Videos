...
annotations_path = "path to dataset"
    ...
    for frame in org_json:
        for key in all_players:
            ...
            for b in range(0, 54, 3):
                x = org_pose[b]
                y = org_pose[b + 1]
                # removing corner keypoints (generated by the pose cleaning script)
...
if __name__ == "__main__":
    ...
    for series_no in SERIES:
        for video in os.listdir(osp.join(annotations_path, series_no)):
            with open(
                osp.join(annotations_path, series_no, video, video + ".json"), "r"
            ) as f:
    ...
    # getting histogram for number of players
