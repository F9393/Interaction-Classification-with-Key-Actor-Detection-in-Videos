...
class PoseReader:
    ...
    def generate_pose_and_masks(self):
        ...
        for penalty_dir in self.X_dirs:
            self.poses = np.zeros(
                (
                    self.num_frames,
                    self.max_players,
                    self.num_keypoints * self.coords_per_keypoint,
                ),
                dtype="float32",
            )
            self.mask = np.ones(
            ...
            with open(
                os.path.join(penalty_dir, f"{os.path.basename(penalty_dir)}.json"), "r"
            ) as f:
        ...
        if self.stage == 'test':
            ...
            with open(os.path.join(self.work_dir, "test_stage.json"), "w") as f:
